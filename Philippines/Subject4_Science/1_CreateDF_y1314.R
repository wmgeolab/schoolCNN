...
hist(df6$science_mean_rs)

range(df6$science_mean_rs)

# Subset columns
final_df6 <- df6[c("school_id","science_mean_rs")]

# Calculate kMeans break with 2 classes
kmeans_df6 <- classIntervals(final_df6$science_mean_rs, 2, style = "kmeans")

# Assign interventon booleans based on above kmeans break 
final_df6$intervention <- NA
final_df6$intervention[final_df6$science_mean_rs < kmeans_df6$brks[2]] <- 1
final_df6$intervention[final_df6$science_mean_rs >= kmeans_df6$brks[2]] <- 0
...
colnames(final_df) <- c("school_id", "science_mean", "intervention", "drop", "latitude", "longitude")

# Rename columns and subset final dataframe
cols <- c("school_id", "science_mean", "intervention", "latitude", "longitude")
