...
files <- list.files("./Brazil/E1_Landsat/data/imagery/", full.names = TRUE)
length(files)

# Read in the data frame and split into pass and fail classes
y1314 <- read.csv("./Brazil/E1_Landsat/data/y15_Brazil.csv")
...
foreach(i = 1:5199) %dopar% {  
	
  id <- base::substr(files[i], 35, 42)
  ...
  if (int == 0) {
      file_name <- paste("./Brazil/E1_Landsat/data/pass/", id, "_", school$intervention, ".jpg", sep = '')
  } else if (int == 1) {
      file_name <- paste("./Brazil/E1_Landsat/data/fail/", id, "_", school$intervention, ".jpg", sep = '')
  }
...
fail_files <- list.files("./Brazil/E1_Landsat/data/fail/", full.names = TRUE)
pass_files <- list.files("./Brazil/E1_Landsat/data/pass/", full.names = TRUE)
