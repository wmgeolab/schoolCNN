...
y_train = train.pop("passing")
x_train = train

y_test = test.pop("passing")
x_test = test
...
# Choose the best performing model and predict each school
to_pred = dta.drop(['passing'], axis = 1)
...
dta = dta.drop(['intervention'], axis = 1)
dta = pd.merge(dta, df, on = 'school_id')
dta = dta.drop(['intervention', 'latitude','longitude'], axis = 1)
dta.head()


final_df = pd.DataFrame()
final_df['school_id'] = dta['school_id']
final_df['actual_mean'] = dta['passing']
...
final_df.to_csv("./Brazil/Ensemble/data/AbsolutePreds.csv")
